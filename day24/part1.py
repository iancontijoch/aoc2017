...
def compute(s: str) -> int:
    ...
    strengths = []
    for start in starts:
        q: Deque[tuple[Component, set[int], int]] = deque(
            [(start, set(), start.strength)],
        )
        max_strength = 0
        while q:
            c1, seen, total_strength = q.popleft()
            if c1.id in seen:
                continue
            max_strength = max(total_strength, max_strength)
            seen.add(c1.id)
            for c2 in (
                ...
                for conn in Component.connect(c1, c2):
                    q.append((
                        conn, seen.copy(), total_strength + c2.strength,
                    ))
